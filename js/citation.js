/* citation.js: build a citation for a register entry. The cited thing is the
   catalogue entry describing an absent work, so the reference names the work,
   its fate, and the register that records it. Returns an HTML form (titles in
   italics) and a plain-text form. Styles: Chicago note, MLA, APA, BibTeX.
   Adapted from the citation builder in MIRL Collate. */

LC.Citation = (function () {
  ...
  function build(record, project, style) {
    const title = LC.Model.title(record);
    ...
    const fate = LC.vocab.statusOf(record.status).label.toLowerCase();
    const reg = has(project.title) ? project.title.trim() : 'Untitled register';
    const comp = has(project.compiler) ? project.compiler.trim() : '';
    const inst = has(project.institution) ? project.institution.trim() : '';
    const entry = 'entry ' + record.id;
